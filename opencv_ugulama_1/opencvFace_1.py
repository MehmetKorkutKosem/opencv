...
while True:
    ...
    if len(contours)> 0:
            ...
            point=np.array([[exTop],[exLeft],[exBottom],[exRight]])
            cv2.polylines(roi,[point],True,(0,0,255),2)
            a=math.sqrt((exRight[0]-exTop[0])**2+(exRight[1]-exTop[1])**2)
            b=math.sqrt((exRight[0]-exBottom[0])**2+(exRight[1]-exBottom[1])**2)
            c=math.sqrt((exBottom[0]-exTop[0])**2+(exBottom[1]-exTop[1])**2)
            ...
            # Hesaplanan açıyı ekrana yazdırıyoruz
                try :
                    ...
                    cv2.putText(roi,str(angle),(exRight[0]-10,exRight[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
                except:
                    cv2.putText(roi,"?",(exRight[0]-10,exRight[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2,cv2.LINE_AA)
            else:
